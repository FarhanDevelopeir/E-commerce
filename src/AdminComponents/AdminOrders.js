...
const AdminOrders = () => {
  const getAllOrders = useSelector(fetchedAllOrders);
  ...
  const handleStatus =(e,item) =>{
    const updateOrder = {...item, status:e.target.value}
    console.log(updateOrder)
    dispatch(updateOrderAsync(updateOrder))
    dispatch(getAllOrdersAsync());
  }

  const filteredOrders = getAllOrders.filter((item) =>
          ...
          {currentOrders.map((item) => {
            return (
              <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                <td class="px-6 py-4">${item.bill}</td>
              </tr>
            );
          })}
        </tbody>
        ...
        <ul className="flex flex-wrap justify-center sm:justify-start -mx-1 my-2">
          ...
          <li className="mx-1">
            <a
              href="#"
              className={`px-3 py-1 text-sm text-gray-500 bg-white border border-gray-300 rounded-md hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white ${
                currentPage === Math.ceil(getAllOrders.length / ordersPerPage)
