            ...
            {!item.addedToCart ? (
              <button
                ...
                onClick={() => {
                  ...
                  dispatch(
                    updateAddedToCart({ productId: item.id, addedToCart: true })
                  );
                }}
              >
                Add to Cart
    ...
    <div>
      <div className="container  border  rounded pt-5 ">
        ...
        <div className="row mb-3">
          {wishlist.length === 0 ? (
            <h1>Sorry there is no Product that you are add to wishlish </h1>
          ) : (
            <div className="row mb-3">{displaywishlist}</div>
          )}
        </div>
      </div>
    </div>
