...
const Filterproducts = () => {
    ...
    const filtercategory = (selectcategory) => {
        const filteredProducts = allfilterproducts.filter((item) => item.category === selectcategory);
        dispatch(filter(filteredProducts))
        setcategoryfilter(true)
    };

    const filterprice = (selectedprice) => {
        const filterprice = allfilterproducts.filter((item) => item.price === selectedprice)
        dispatch(filter(filterprice))
        setcategoryfilter(true)
    }

    const displayproductsinfilteration = categoryfilter === true ? filterproduct : allfilterproducts
    ...
    const filteredProducts = displayproductsinfilteration.filter((item) => {
        ...
        return (
            productTitle.includes(search) || productCategory.includes(search)
        )
    ...
    const displaydata = nomatchesproducts ? (<h2 className='mt-5 text-center' >Sorry, data is not available</h2>) : (
            ...
            <div className='container d-flex'>
                <div className='' style={{ width: '20%', marginTop: '10px' }}>
                    ...
                    {Category.map((item) => {
                        return (<>
                            <label>
                                <input type='radio' onChange={(e) => filtercategory(item)} class="form-check-input" name='category' />
                                {item}
                            </label><br />
                        </>
                        )
                    ...
                    <button className='btn btn-danger mt-3' name='category' onClick={() => { setcategoryfilter(false); setSearchInput(''); }}>Clear filters</button>
